 ...
 * which will then be used by the `iconv()` function in R.
 *
...
SEXP pxlib_get_data_c(SEXP pxdoc_extptr) {
  ...
  for (int j = 0; j < num_fields; j++) {
    // ИСПРАВЛЕНО: Просто создаем R-строку из необработанного имени поля.
    // Перекодировка теперь будет выполняться на уровне R.
